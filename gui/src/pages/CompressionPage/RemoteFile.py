# Leave this note here for future reference.
#
# This file should be kept around because it almost worked,
# and there were various tweaks that were needed.
#
# I *ALMOST* got h5py on remote files working using this class but for some
# reason h5py doesn't seem to be happy when being passed a file-like object. I
# can't find a solution to this so I am going to need to resort to loading the
# data on the javascript end, which is a shame.
#
# The error in the browser is the following:
#
# useCoeffSizes.ts:42 PythonError: Traceback (most recent call last):
#   File "/lib/python312.zip/_pyodide/_base.py", line 597, in eval_code_async
#     await CodeRunner(
#   File "/lib/python312.zip/_pyodide/_base.py", line 411, in run_async
#     coroutine = eval(self.code, globals, locals)
#                 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#   File "_script.py", line 256, in <module>
#     test_compression(
#   File "_script.py", line 32, in test_compression
#     h5f = h5py.File(f, mode="r")
#           ^^^^^^^^^^^^^^^^^^^^^^
#   File "/lib/python3.12/site-packages/h5py/_hl/files.py", line 561, in __init__
#     fid = make_fid(name, mode, userblock_size, fapl, fcpl, swmr=swmr)
#           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#   File "/lib/python3.12/site-packages/h5py/_hl/files.py", line 235, in make_fid
#     fid = h5f.open(name, flags, fapl=fapl)
#           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#   File "h5py/_objects.pyx", line 54, in h5py._objects.with_phil.wrapper
#   File "h5py/_objects.pyx", line 55, in h5py._objects.with_phil.wrapper
#   File "h5py/h5f.pyx", line 102, in h5py.h5f.open
# OSError: Unable to open file (attempting I/O in temporary file space)
#
# That was using this code in Pyodide
#
